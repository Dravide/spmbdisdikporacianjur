<x-Pendaftaran.apps title="Berkas Khusus">
    @if((Auth::user()->dataPendaftar->konfir == 2 or Auth::user()->dataPendaftar->konfir == 0))
        @if($d)
            @foreach($d as $khusus)
                <x-Pendaftaran.berkas berkas="{{ $khusus->nama_berkas }}" ekstensi=".jpg, .png, .gif dan .pdf"
                                      id="{{ $khusus->id }}">
                    <x-slot name="icon">
                        {{ $khusus->svg }}
                    </x-slot>
            ...
            @foreach($berkastambahan as $data)
    ...
    @push('js')
        ...
        <script
            src="https://unpkg.com/filepond-plugin-pdf-preview/dist/filepond-plugin-pdf-preview.min.js"></script>
        <script
