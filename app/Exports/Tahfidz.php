...
class Tahfidz implements WithProperties, WithCustomStartCell, FromQuery, WithHeadings, WithMapping, WithColumnFormatting, ShouldAutoSize, WithEvents, WithColumnWidths
{
        ...
        return [
            AfterSheet::class => function (AfterSheet $event) {
                ...
                $event->sheet->getDelegate()->getStyle('A1:I1')
                    ...
                    ->getColor()
                    ->setARGB('F5F7F8');
                $sheet->getDelegate()->getStyle('A1:I1')->getFont()->setSize('20')->setBold(true);
                $sheet->getDelegate()->getStyle('A1:I1')->getFill()
                ...
                $event->sheet->getDelegate()->getStyle('A2:I2')
                    ...
                    ->getColor()
                    ->setARGB('F5F7F8');
                $sheet->getDelegate()->getStyle('A2:I2')->getFill()
                    ->setFillType(\PhpOffice\PhpSpreadsheet\Style\Fill::FILL_SOLID)
                    ->getStartColor()
                    ->setARGB('45474B');
                ...
                $event->sheet->getDelegate()->getStyle('A3:I3')
                    ...
                    ->getColor()
                    ->setARGB('F5F7F8');
                $sheet->getDelegate()->getStyle('A3:I3')->getFont()->setSize('16')->setBold(true);
                $sheet->getDelegate()->getStyle('A3:I3')->getFill()
                    ->setFillType(\PhpOffice\PhpSpreadsheet\Style\Fill::FILL_SOLID)
                    ->getStartColor()
                    ->setARGB('45474B');
                $sheet->mergeCells('A5:B5');
                $sheet->setCellValue('A5', "Jalur Pendafataran");
                $sheet->setCellValue('C5', ": Prestasi Tahfidz");
                ...
                $sheet->setCellValue('C8', ": " . DataPendaftar::whereIdSekolah(\Auth::user()->sekolah->id)->whereIdJalur('5')->count());
